PERSUADE <- function(years, status, group, strata = FALSE, time_unit, 
  ...
  #km
  km  <- npsurv(form)
  km_names <- c(rep(1, km$strata[1]), if (ngroups>1) {rep(2, km$strata[2])} else {NA},
                if (ngroups>2) {rep(3, km$strata[3])} else {NA})
  ...
  l1 <- list("group" = group, "ngroups" = ngroups, "group_names" = group_names, "show_spline" = show_spline, 
             ...
             "form" = form, "km" = km, "km_names" = km_names, "hr_smooth1" = hr_smooth1, "hr_names" = hr_names, 
             "cox_reg" = cox_reg, "expo" = expo, "weib" = weib, "gom" = gom, "lnorm" = lnorm, "llog" = llog, 
             "gam" = gam, "ggam" = ggam, "expo_pred" = expo_pred, "weib_pred" = weib_pred, "gom_pred" = gom_pred, 
             "gom_est_h" = gom_est_h, "lnorm_pred" = lnorm_pred, "llog_pred" = llog_pred, "gam_pred" = gam_pred, 
             "ggam_pred" = ggam_pred, "lbls" = lbls, "IC" = IC, "extrapolation_gr_1" = extrapolation_gr_1, 
             "tp_gr_1" = tp_gr_1, "cols_extr" = cols_extr)
  if (ngroups>1) {l2 <- list("hr_smooth2" = hr_smooth2, "extrapolation_gr_2" = extrapolation_gr_2, 
                             "tp_gr_2" = tp_gr_2)} else {l2 <- NA}
  if (ngroups>2) {l3 <- list("hr_smooth3" = hr_smooth3, "extrapolation_gr_3" = extrapolation_gr_3, 
                             "tp_gr_3" = tp_gr_3)} else {l3 <- NA}
  if (spline_mod == TRUE) {l4 <- list("spl_hazard1" = spl_hazard1, "spl_hazard2" = spl_hazard2, 
                                      "spl_odds1" = spl_odds1, "spl_odds2" = spl_odds2, "spl_normal1" = spl_normal1, 
                                      "spl_normal2" = spl_normal2, "spl_hazard1_pred" = spl_hazard1_pred, 
                                      "spl_hazard2_pred" = spl_hazard2_pred, "spl_odds1_pred" = spl_odds1_pred, 
                                      "spl_odds2_pred" = spl_odds2_pred, "spl_normal1_pred" = spl_normal1_pred, 
                                      "spl_normal2_pred" = spl_normal2_pred, "IC_spl" = IC_spl, 
                                      "lbls_spline" = lbls_spline)} else {l4 <- NA}
