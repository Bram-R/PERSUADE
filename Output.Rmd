...
---
title: "PERSUADE `r paste0(PERSUADE$name)`"
date: "`r Sys.Date()`"
---
...
kable_styling(
  kable(
    ...
    format = "latex", booktabs = TRUE, linesep = "",
    caption = "Cure proportions per group"),
  latex_options = c("HOLD_position", "scale_down", "striped"))
```
...
```{r validate_extrapolation1, echo=FALSE, out.height="29%", fig.width=11}
i <- 1
kable_styling(
  kable(
    t(round(PERSUADE$surv_pred$gr[[i]][1 + PERSUADE$input$time_pred_surv_table, ], 3))[-1, ], 
    col.names = paste("T=", PERSUADE$surv_pred$gr[[i]][1 + PERSUADE$input$time_pred_surv_table, ][, 1]), 
    format = "latex", booktabs = TRUE, linesep = "",
...
kable_styling(
  kable(
    f_summary(PERSUADE$surv_pred$tp_gr[[i]][, 2:PERSUADE$misc$cols_tp]), 
    format = "latex", booktabs = TRUE, linesep = "", 
...
### `r paste("Group", PERSUADE$misc$group_names[i + 1])`
```{r validate_extrapolation2, echo=FALSE, out.height="29%", fig.width=11, eval=if (PERSUADE$misc$ngroups>1) {TRUE} else {FALSE}}
i <- i + 1
kable_styling(
  kable(
    t(round(PERSUADE$surv_pred$gr[[i]][1 + PERSUADE$input$time_pred_surv_table, ], 3))[-1, ], 
...
### `r paste("Group", PERSUADE$misc$group_names[i + 1])`
```{r validate_extrapolation3, echo=FALSE, out.height="29%", fig.width=11, eval=if (PERSUADE$misc$ngroups>2) {TRUE} else {FALSE}}
i <- i + 1
kable_styling(
  kable(
    t(round(PERSUADE$surv_pred$gr[[i]][1 + PERSUADE$input$time_pred_surv_table, ], 3))[-1, ], 
